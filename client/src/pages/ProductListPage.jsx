...
const ProductListPage = () => {
  ...
  const navigate = useNavigate(); // useNavigate එක initialize කරන්න
  ...
  const fetchProductdata = async () => {
    try {
      ...
      if (responseData.success) {
        ...
        navigate("/"); // Invalid response එකක් නම් home page redirect
      }
    } catch (error) {
      AxiosToastError(error);
      navigate("/"); // Error එකක් ඇති විට home page redirect
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    // categoryId සහ subCategoryId වල වාදිකතාව පරීක්ෂා කරන්න
    if (!categoryId || !subCategoryId) {
      navigate("/"); // අනීතික URLs සඳහා home පිටුවට යන්න
    } else {
      fetchProductdata();
    }
  }, [params]);
