...
const CheckoutPage = () => {
  const { notDiscountTotalPrice, totalPrice, fetchCartItem, fetchOrder } = useGlobalContext()
  ...
  // Redirect to the home page if refreshed while on CheckoutPage
  useEffect(() => {
    const isCheckoutPageVisited = sessionStorage.getItem('isCheckoutPageVisited')

    if (isCheckoutPageVisited === 'true') {
      sessionStorage.removeItem('isCheckoutPageVisited')
      navigate('/')
    } else {
      sessionStorage.setItem('isCheckoutPageVisited', 'true')
    }
  ...
  const totalQuantity = cartItemsList.reduce((sum, item) => sum + (item.quantity || 0), 0)
    ...
    try {
      const response = await Axios({
        ...
        data: {
          ...
          totalAmt: totalPrice,
          totalQuantity: totalQuantity // Add totalQuantity to the request
        },
      })
